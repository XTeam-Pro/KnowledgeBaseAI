...
7. [P5 — Learner State Graph](#7-p5--learner-state-graph)
...
**Отдельный слой** данных — **Learner State Graph** — поверх статического Knowledge Graph. Два варианта реализации:

#### Вариант A: Всё в Neo4j (рекомендуется для текущего масштаба)

```
Статический слой (уже есть):
  Subject → Section → Subsection → Topic → Skill → Method → Example

Динамический слой (новый):
  (:User)-[:MASTERED]->(Skill)
  (:User)-[:ATTEMPTED]->(Example)
  (:User)-[:STUDYING]->(Topic)
  (:User)-[:STRUGGLING_WITH]->(Misconception)
```

**Плюсы**: один запрос связывает ученика с графом знаний.
**Минусы**: пишем состояние ученика в ту же БД, что и статический граф.

#### Вариант B: Отдельный Neo4j instance для learner state

При масштабе >10K активных учеников — отдельный Neo4j instance:
- **KB Neo4j**: только статический граф знаний (read-heavy, редкие мутации)
- **Learner Neo4j**: пользовательские данные (write-heavy, каждый ответ = запись)

Для текущей стадии проекта рекомендуется **Вариант A** (один Neo4j).

### 7.3 Модель данных

#### 7.3.1 Новый тип узла: User

```
(:User {
    uid: String,              // "USR-{user_id}"
    tenant_id: String,
    external_id: String,      // ID из StudyNinja-API (PostgreSQL)
    user_class: Integer,      // Класс ученика (2–11)
    created_at: DateTime
})
```

> **Примечание**: User-узел содержит только ID и класс. Все персональные данные остаются в PostgreSQL StudyNinja-API. Neo4j хранит только граф-связи.

#### 7.3.2 Новые типы связей

```
(:User)-[:MASTERED {
    score: Float,              // 0.0–1.0
    tier: String,              // "novice" | "proficient" | "expert" | "olympiad"
    last_practiced: DateTime,  // для spaced repetition
    practice_count: Integer,   // сколько раз практиковал
    updated_at: DateTime
}]->(:Skill)

(:User)-[:ATTEMPTED {
    result: String,            // "correct" | "incorrect" | "partial" | "skipped"
    time_spent_sec: Integer,
    attempt_number: Integer,   // попытка (1, 2, 3...)
    misconception_uid: String, // если обнаружена misconception
    timestamp: DateTime
}]->(:Example)

(:User)-[:STUDYING {
    status: String,            // "not_started" | "in_progress" | "completed" | "reviewing"
    started_at: DateTime,
    completed_at: DateTime,
    mastery: Float             // текущий mastery по теме
}]->(:Topic)

(:User)-[:STRUGGLING_WITH {
    occurrence_count: Integer, // сколько раз зафиксирована эта ошибка
    first_seen: DateTime,
    last_seen: DateTime,
    resolved: Boolean          // ученик преодолел ошибку
}]->(:Misconception)
```

### 7.4 Ключевые запросы

#### 7.4.1 Collaborative filtering — «похожие ученики»

```cypher
// Найти учеников, которые ошибались в тех же местах, что текущий ученик
MATCH (me:User {uid: $user_uid})-[:STRUGGLING_WITH]->(m:Misconception)<-[:STRUGGLING_WITH]-(other:User)
WHERE other.uid <> $user_uid
WITH other, count(m) AS shared_misconceptions
ORDER BY shared_misconceptions DESC
LIMIT 10

// Найти темы, которые эти «похожие» ученики успешно прошли после наших общих ошибок
MATCH (other)-[:STUDYING {status: 'completed'}]->(t:Topic)
WHERE NOT EXISTS {
    MATCH ($user_uid_node:User {uid: $user_uid})-[:STUDYING {status: 'completed'}]->(t)
}
RETURN t.uid, t.title, count(other) AS recommended_by
...
```cypher
MATCH (u:User {uid: $user_uid})-[m:MASTERED]->(sk:Skill)
WHERE m.last_practiced < datetime() - duration({days: $review_interval})
  AND m.score < 0.9  // Не мастерски освоенные навыки быстрее забываются
RETURN sk.uid, sk.title, m.score, m.last_practiced,
       duration.between(m.last_practiced, datetime()).days AS days_since_practice
...
```cypher
MATCH (u:User {uid: $user_uid})-[s:STUDYING]->(t:Topic)
WHERE s.status IN ['completed', 'in_progress']
OPTIONAL MATCH (t)-[:PREREQ]->(dep:Topic)<-[sd:STUDYING]-(u)
RETURN t.uid, t.title, s.status, s.mastery, s.started_at, s.completed_at,
       collect({dep_uid: dep.uid, dep_title: dep.title, dep_mastery: sd.mastery}) AS prereq_progress
...
```cypher
// Самые сложные темы для класса 7 (больше всего STRUGGLING_WITH)
MATCH (u:User {user_class: 7})-[sw:STRUGGLING_WITH]->(m:Misconception)-[:COMMON_IN]->(t:Topic)
WHERE sw.resolved = false
RETURN t.uid, t.title,
       count(DISTINCT u) AS struggling_students,
       collect(DISTINCT m.title) AS common_misconceptions
...
async def _update_learner_graph(
    ...
    """Обновить Learner State Graph в Neo4j."""
    # 1. ATTEMPTED связь
    await self._kb_repo.write(
        """
        MERGE (u:User {uid: $user_uid})
        WITH u
        MATCH (ex:Example {uid: $example_uid})
        CREATE (u)-[:ATTEMPTED {
            result: $result,
            time_spent_sec: $time_spent,
            misconception_uid: $misconception_uid,
            timestamp: datetime()
        }]->(ex)
        """,
        {
    ...
    # 2. Обновить MASTERED связь на Skill
    await self._kb_repo.write(
        """
        MERGE (u:User {uid: $user_uid})
        WITH u
        MATCH (sk:Skill {uid: $skill_uid})
        MERGE (u)-[m:MASTERED]->(sk)
        SET m.score = $mastery,
            m.last_practiced = datetime(),
            m.practice_count = coalesce(m.practice_count, 0) + 1,
            m.updated_at = datetime(),
            m.tier = CASE
                WHEN $mastery >= 0.9 THEN 'olympiad'
                WHEN $mastery >= 0.75 THEN 'expert'
                WHEN $mastery >= 0.5 THEN 'proficient'
                ELSE 'novice'
            END
        """,
        {"user_uid": user_uid, "skill_uid": skill_uid, "mastery": new_mastery},
    )

    # 3. Обновить STRUGGLING_WITH если есть misconception
    if misconception_uid:
        await self._kb_repo.write(
            """
            MERGE (u:User {uid: $user_uid})
            WITH u
            MATCH (m:Misconception {uid: $misc_uid})
            MERGE (u)-[sw:STRUGGLING_WITH]->(m)
            SET sw.occurrence_count = coalesce(sw.occurrence_count, 0) + 1,
                sw.last_seen = datetime(),
                sw.first_seen = coalesce(sw.first_seen, datetime())
            """,
            {"user_uid": user_uid, "misc_uid": misconception_uid},
        )
```

### 7.6 Privacy и разделение данных

- User-узлы в Neo4j содержат **только uid и user_class** — без имён, email, паролей
- Все персональные данные остаются в PostgreSQL StudyNinja-API
- При удалении аккаунта → каскадное удаление User-узла и всех его связей из Neo4j
- GDPR compliance: `DELETE (u:User {uid: $uid}) DETACH DELETE u`

### 7.7 Критерии завершения

- [ ] User-узлы создаются в Neo4j при первом взаимодействии с KB
- [ ] ATTEMPTED, MASTERED, STUDYING, STRUGGLING_WITH связи записываются при каждом событии
...
- [ ] Cascade delete User при удалении аккаунта
...
WHERE ex.bloom_level = $target_bloom_level
  ...
  AND NOT EXISTS {
      MATCH (:User {uid: $user_uid})-[:ATTEMPTED]->(ex)
  }
RETURN ex
ORDER BY rand()
LIMIT 1
...
class ScaffoldingEngine:
    """
    Определяет текущий уровень сложности ученика
    и выбирает следующий пример с учётом прогресса.
    """
    ...
    async def get_next_example(
        ...
        """Выбрать следующий пример с учётом текущего уровня."""

        # 1. Получить текущий tier ученика
        current_tier = await self._get_current_tier(user_uid, topic_uid, tenant_id)

        # 2. Получить примеры текущего tier, которые ученик ещё не решал
        query = """
        MATCH (dt:DifficultyTier {uid: $tier_uid})-[:TIER_EXAMPLE]->(ex:Example)
        WHERE NOT EXISTS {
            MATCH (:User {uid: $user_uid})-[:ATTEMPTED {result: 'correct'}]->(ex)
        }
        ...
        # 3. Все примеры текущего tier решены → проверить, разблокирован ли следующий
        next_tier = await self._get_next_tier(current_tier["uid"], tenant_id)
        if next_tier:
            mastery = await self._get_tier_mastery(user_uid, current_tier["uid"])
            if mastery >= current_tier.get("target_mastery", 0.7):
    ...
    async def on_incorrect_answer(
        ...
        """
        При неправильном ответе — откат на более простой пример
        того же или предыдущего уровня.
        """
        # Попробовать более простой пример текущего tier
        query = """
        MATCH (dt:DifficultyTier {uid: $tier_uid})-[:TIER_EXAMPLE]->(ex:Example)
        WHERE ex.difficulty_level < $max_diff
          AND NOT EXISTS {
              MATCH (:User {uid: $user_uid})-[a:ATTEMPTED]->(ex)
        ...
        prev_query = """
        MATCH (prev:DifficultyTier)-[:NEXT_TIER]->(dt:DifficultyTier {uid: $tier_uid})
        RETURN prev.uid AS uid
        """
        prev = await self._repo.read(prev_query, {"tier_uid": current_tier_uid})
        if prev:
            return await self.get_next_example(user_uid, topic_uid, tenant_id)
...
| MASTERED | P5 | User→Skill | Динамически |
| ATTEMPTED | P5 | User→Example | Динамически |
| STUDYING | P5 | User→Topic | Динамически |
| STRUGGLING_WITH | P5 | User→Misconception | Динамически |
...
```
Фаза 0 (неделя 1): P0 — Orphan-сущности
  Нулевой риск, чистка данных, CI-валидатор.

Фаза 1 (недели 2–3): P1 — Weighted PREREQ
  Обратно совместимо: weight=0.5 по умолчанию = поведение не меняется.
  Постепенный переход roadmap planner на взвешенную модель.

Фаза 2 (недели 4–6): P2 — Misconception + P3 — Skill Composition
  Независимы друг от друга, можно параллельно.
  Новые типы узлов, не ломают существующие.

Фаза 3 (недели 7–9): P4 — Neo4j Vector + P6 — Bloom
  P4 требует аккуратной миграции Qdrant → Neo4j.
  P6 — автоклассификация + ручная проверка.

Фаза 4 (недели 10–12): P5 — Learner State Graph
  Зависит от P2 (Misconception) для STRUGGLING_WITH.
  Основная интеграция с StudyNinja-API.

Фаза 5 (недели 13–16): P8 — Difficulty Micro-Graph + P9 — Versioning
  Зависит от P6 (Bloom) для полной картины сложности.
  P9 — инфраструктурный, можно реализовать в любой момент.

Фаза 6 (недели 17–20): P7 — Multi-Subject
  Самый масштабный, требует контента для второго предмета.
  Зависит от зрелости всех предыдущих фаз.
```

### 13.2 Граф зависимостей фаз
...
P2 (Misconception) ──────────┐                    │ │
                             ├── P5 (Learner) ────┤ │
P3 (Skill Composition) ─────┘                     │ │
                                                   │ │
P4 (Neo4j Vector) ───────────────────────────────┤ │
                                                  │ │
P6 (Bloom) ──────────── P8 (Difficulty Tiers) ───┤ │
                                                  │ │
P9 (Versioning/AB) ──────────────────────────────┤ │
                                                  │ │
                        P7 (Multi-Subject) ◀──────┘ │
...
Для каждой фазы:
1. Перед началом — snapshot текущего состояния (P9 или ручной дамп)
2. Все изменения canonical spec — через feature flags
3. При откате: удалить новые узлы/связи, вернуть canonical spec
...
После реализации всех 10 улучшений граф знаний трансформируется из **статического каталога тем** в **живую адаптивную систему**, которая:
